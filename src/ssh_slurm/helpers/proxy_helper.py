...
def suggest_port_forwarding(host: str, ssh_config_path: str | None = None):
    ...
    print("\nTo use this host with ssb, consider these alternatives:")
    ...
    print(
        f"   s2 add {host}-direct <real_hostname> {ssh_host.effective_user} {ssh_host.effective_identity_file}"
    )

    print("\n3. Use SSH config with a bastion setup:")
    print(
        "   Configure your SSH client to handle ProxyJump automatically outside of ssb"
    )
...
def main():
    ...
    if ssh_host.proxy_jump:
        ...
        print(f"\nHost '{args.host}' should work directly with ssb.")

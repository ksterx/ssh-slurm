...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "profile":
        ...
        parser = argparse.ArgumentParser(
            prog="ssb profile",
            description="Manage SSH SLURM server profiles"
        )
        parser.add_argument(
            "--config", help="Config file path (default: ~/.config/ssh-slurm.json)"
        )
        
        subparsers = parser.add_subparsers(dest="profile_command", help="Profile commands")
        ...
        add_parser.add_argument("--ssh-host", help="SSH config host name (from ~/.ssh/config)")
        
        # Direct connection parameters group
        direct_group = add_parser.add_argument_group("Direct connection (use when not using --ssh-host)")
        ...
        show_parser.add_argument("name", nargs="?", help="Profile name (default: current)")

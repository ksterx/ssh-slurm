...
def handle_profile_command(args):
    ...
    if not args.profile_command:
        ...
        print("Use 'ssb profile --help' for available commands", file=sys.stderr)
        sys.exit(1)
...
def cmd_list(args, config_manager: ConfigManager):
    ...
    for name, profile in profiles.items():
        ...
        # Show whether it uses SSH config or direct connection
        if profile.ssh_host:
            conn_info = f"ssh:{profile.ssh_host} ({profile.username}@{profile.hostname}:{profile.port})"
        else:
            conn_info = f"{profile.username}@{profile.hostname}:{profile.port}"

        desc = f" - {profile.description}" if profile.description else ""
        print(f"{marker} {name}: {conn_info}{desc}")
...
def cmd_update(args, config_manager: ConfigManager):
    ...
    if args.ssh_host:
        ...
        updates["hostname"] = ssh_host.effective_hostname
        updates["username"] = ssh_host.effective_user
        updates["key_filename"] = ssh_host.effective_identity_file
        updates["port"] = ssh_host.effective_port
